...
def test_non_admin_cannot_create(other_client):
    r = other_client.post(
        "/api/automations",
        json={"name": "x", "trigger": "lead_created", "action": "send_notification",
...
def test_change_stage_requires_valid_stage_and_deal_trigger(admin_client):
    ...
    r = admin_client.post(
        "/api/automations",
        json={"name": "bad", "trigger": "deal_created", "action": "change_stage",
              "action_config": {}},
    )
    ...
    r = admin_client.post(
        "/api/automations",
        json={"name": "bad2", "trigger": "lead_created", "action": "change_stage",
    ...
    r = admin_client.post(
        "/api/automations",
        json={"name": "ok", "trigger": "deal_created", "action": "change_stage",
              "action_config": {"to_stage": "qualified"}},
    )
    assert r.status_code == 201, r.text
...
def test_update_and_delete(admin_client):
    rid = admin_client.post(
        "/api/automations",
        json={"name": "n", "trigger": "deal_won", "action": "send_notification",
...
async def test_event_create_task_fires(auto_session, db: Session, test_org, admin_user):
    lead = Lead(organization_id=test_org.id, first_name="A", last_name="A",
                stage=LeadStage.new, owner_id=admin_user.id)
    db.add(lead)
    db.commit()
    db.refresh(lead)
    rule = _seed_rule(
    ...
    runs = db.execute(
        text("SELECT status FROM automation_runs WHERE rule_id = :r"), {"r": rule.id}
    ).scalars().all()
    assert runs == ["success"]


async def test_event_is_idempotent(auto_session, db: Session, test_org, admin_user):
    lead = Lead(organization_id=test_org.id, first_name="B", last_name="B",
                stage=LeadStage.new, owner_id=admin_user.id)
    db.add(lead)
    db.commit()
    db.refresh(lead)
...
async def test_disabled_rule_does_nothing(auto_session, db: Session, test_org, admin_user):
    lead = Lead(organization_id=test_org.id, first_name="C", last_name="C",
                stage=LeadStage.new, owner_id=admin_user.id)
    db.add(lead)
    db.commit()
    db.refresh(lead)
...
async def test_change_stage_action(auto_session, db: Session, test_org, admin_user):
    deal = Deal(organization_id=test_org.id, title="D", value=Decimal("100"),
                currency=Currency.EUR, stage=DealStage.new, owner_id=admin_user.id)
    db.add(deal)
    db.commit()
    db.refresh(deal)
    rule = _seed_rule(
...
async def test_stale_scan_fires_once_per_day(auto_session, db: Session, test_org, admin_user):
    ...
    stale = Lead(organization_id=test_org.id, first_name="Old", last_name="Lead",
                 stage=LeadStage.new, owner_id=admin_user.id)
    stale.updated_at = old
    db.add(stale)
    # A fresh lead that must NOT trigger.
    db.add(Lead(organization_id=test_org.id, first_name="Fresh", last_name="Lead",
                stage=LeadStage.new, owner_id=admin_user.id))
    db.commit()
    rule = _seed_rule(
        db, test_org.id,
