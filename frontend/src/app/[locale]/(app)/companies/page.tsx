...
export default function CompaniesPage() {
  ...
  const companiesQuery = useCompaniesInfinite(debouncedQ);
  const items = useMemo(
    () => companiesQuery.data?.pages.flatMap((p) => p.items) ?? [],
    [companiesQuery.data],
  );
  const ids = useMemo(() => items.map((c) => c.id), [items]);
  const tagsQuery = useCompanyTags(ids);
  const tagMap = tagsQuery.data ?? {};

  const deleteCompany = useDeleteCompany();
  const error =
    (companiesQuery.isError && (companiesQuery.error as Error).message) ||
    (deleteCompany.isError && (deleteCompany.error as Error).message) ||
    null;
  ...
  async function handleDelete(company: Company) {
    ...
    try {
      ...
      /* surfaced via deleteCompany.isError */
    }
  }
    ...
    <div className="space-y-4">
      ...
      {selected.size > 0 && (
        <BulkTagBar
          ...
          onApplied={() => qc.invalidateQueries({ queryKey: companiesKeys.all })}
      ...
      {companiesQuery.hasNextPage && (
        <div className="flex justify-center">
          ...
          >
            {companiesQuery.isFetchingNextPage ? tCommon("loading") : tCommon("loadMore")}
          </Button>
        </div>
      )}
    </div>
