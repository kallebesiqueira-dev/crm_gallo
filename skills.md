...
> **Last reviewed:** 2026-06-11 (Deal detail page + edit, Hoje sidebar badge, Products frontend all done)
...
- **Products / Services catalog — DONE:** `products` org-scoped tenant table (RLS ENABLE+FORCE+GUC policy) + `producttype` enum via migration `d7e8f9a0b1c2`; `Product` model, `ProductCreate/Update/Out` schemas, full CRUD `/api/products`. Frontend (list/new/edit + sidebar entry + 7-locale i18n) all done 2026-06-11.
...
- ~~Audit SELECT strict policy~~ DONE 2026-06-10 (`3adffee` + lint fixup `6fbcc42`) — migration `9f8e7d6c5b4a` drops the "unset GUC reads everything" branch; cross-tenant audit reads are now closed at the DB, not just app-layer WHERE clauses. Unblocked by `implicit_returning=False` on `AuditLog` (plain INSERTs — the RETURNING re-check was why the policy had stayed permissive since 2026-05-30). 5 crm_app-role probes in `tests/test_audit_rls_strict.py`; the repo-grep-guard alternative is moot.
...
- [x] Existing logged-in user accepting an invite to join a NEW org — `POST /api/auth/accept-invite` (authenticated, email must match). Done 2026-06-10.
- [x] Resend / regenerate invite endpoint — `POST /api/orgs/current/invites/{id}/resend` (rotates token+expiry if expired). Done 2026-06-10.
- [x] Background sweep deleting expired-and-unaccepted invites older than 30 days — `prune_expired_invites` cron job daily at 03:17. Done 2026-06-10.
...
- [x] Audit_logs SELECT policy is RLS-strict — migration 9f8e7d6c5b4a + `AuditLog(implicit_returning=False)`. Done 2026-06-10.
...
- [x] DB-load of user on /refresh to enforce `is_active=false` instantly — done (auth.py:641-662, re-loads from DB at every refresh). Done ≥2026-06-09.
...
- [x] **MFA mandatory for admin/manager** — `MFA_REQUIRED_FOR_PRIVILEGED` config flag, gate at login (auth.py:368). Done ≥2026-06-05.
- [x] **MFA secret at-rest encryption** — `EncryptedSecret(255)` Fernet column type on `users.mfa_secret`. Done ≥2026-06-06.
...
- [x] **Sentry source-map upload** — DONE 2026-06-12 (`ebd28cb`): `withSentryConfig` em `next.config.ts`, inerte sem `SENTRY_AUTH_TOKEN` (dev/CI inalterados, build verificado). **Ação do usuário:** criar o auth token no Sentry e setar `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` no build env do Railway (documentado no `.env.example`).
- [x] **Grafana dashboard JSON** committed to repo — `monitoring/grafana-dashboard.json`. Done 2026-06-10.
...
- [x] **Dead-letter queue + alert** — `dlq_wrap` in `worker/settings.py` pushes terminal failures to `arq:dead`; `arq_dead_queue_depth` Prometheus gauge + `DLQBacklog` alert rule. Done 2026-06-10.
...
- [ ] **Webhook follow-ups (P2/P3)**: `POST /{id}/rotate-secret` (intentional + audited; today secret rotation requires a recreate); `POST /{id}/test` manual ping endpoint; receiver-side reference middleware as a docs example; ~~admin UI page on `/settings`~~ (done 2026-06-11 — `WebhooksCard` in settings + 7-locale i18n); `webhook_deliveries_total` + `webhook_delivery_duration_seconds` Prometheus metrics; retention prune for `webhook_deliveries` (90d default); test isolation for `deliver_webhook` success/auto-pause paths (currently smoke-only — see comment in `tests/test_webhooks.py`).
...
- [x] **Search v2** — `pg_trgm` extension (migration `7a8b9c0d1e2f`) + GIN trigram indexes + similarity fallback in leads/customers API (if FTS returns 0 hits, retries with `similarity() > 0.3`). Done 2026-06-11. Deals search uses `ILIKE` (title is short, exact match is fine). Remaining P3: ts_rank in response, per-locale dictionary swap.
...
- [x] **Optimistic locking on Task + Customer** — both have `version` + `check_if_match`. `Lead` version column added 2026-06-10 (migration `a1b2c3d4e5f6`).
- [x] **Strict mode**: `check_if_match` already raises 428 on missing header (not just a warning). Done 2026-06-04.
- [x] **DLQ for send_email** — `dlq_wrap(send_email)` in worker/settings.py. Done 2026-06-10.
- [x] Stable sort key (`(created_at DESC, id DESC)`) — `paginate()` applies it; deals list got `id DESC` tiebreaker 2026-06-10.
- [x] N+1 audit: schemas expose only `owner_id` UUIDs (no nested objects), so no lazy-load N+1 on list endpoints.
- [x] **Deals search** — `q` param + `title.ilike` added 2026-06-10.

#### Rate limiting beyond /login
- [x] `/api/auth/register` (5/min/IP) — done ≥2026-06-05.
- [x] `/api/assistant/chat` (`rate_limit_assistant_per_minute`/min/user) — done 2026-06-10.
- [x] `/api/leads/{id}/score` (`rate_limit_score_per_hour`/hour/user) — done ≥2026-06-05.
- [x] Imports endpoint (1 concurrent/org + 24h rolling cap) — done ≥2026-06-03.
- [x] Webhook receiver endpoints (600/min per-IP Redis INCR+EXPIRE) — done 2026-06-10.
...
- [x] Cache `/api/dashboard/stats` in Redis (60s TTL, invalidate on mutation) — done 2026-06-10.
- [x] Trash list pagination (currently loads all soft-deleted rows) — done 2026-06-10.
