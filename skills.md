...
- [ ] **Strict mode**: now that all three entities echo `version` (customers/tasks via PATCH, deals via `/move`), `check_if_match` can flip from v1-lenient to header-required. Before flipping, confirm via the `{entity}.if_match_missing` log volume that no live client still omits it; then drop the lenient branch in `app/api/_concurrency.py` and assert the missing-header rejection in `tests/test_resource_locking.py`.
