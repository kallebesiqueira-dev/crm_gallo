...
- [x] **Frontend echoes `version` (§322a) — DONE 2026-06-04.** `frontend/src/lib/api.ts`: `version: number` added to the `Customer`/`Deal`/`Task` interfaces; `updateCustomer`/`updateDeal`/`updateTask` take an optional `version?` param and send `If-Match: String(version)` only when defined (version 0 is sent — `!== undefined` guard, not truthiness). UI wired for customer-edit page + tasks page: capture `version` on load, on 412 show the `common.versionConflict` i18n string (7 locales) and reload the fresh row/list. `updateDeal` has the header support but **no UI caller yet** (deals board still doesn't pass version). vitest `tests/api.test.ts` "optimistic locking (If-Match)" block, 22/22 green. tsc + eslint clean.
- [ ] **Strict mode**: flip `check_if_match` to require the header once the frontend echoes `version` on *every* PATCH. Customers + tasks now send it; **deals UI still does not call `updateDeal` with a version** — wire the deal board/edit first, then flip. Server currently logs `{entity}.if_match_missing` as the rollout signal.
