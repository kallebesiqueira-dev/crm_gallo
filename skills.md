...
- [x] **`crm_app` password de-hard-coded + rotated — DONE 2026-06-08.** The migration no longer ships a literal password; it is sourced from `APP_DATABASE_URL` / `CRM_APP_DB_PASSWORD` (`f5fde59e0dc8` + rotation migration `0862af48f9ca`, which `ALTER ROLE`s existing DBs). The old value is in git history → treat as compromised: rotate `APP_DATABASE_URL` per environment (incl. Railway prod) and redeploy.
- [x] **`.env.example` scrubbed of concrete credentials — DONE 2026-06-05.** The git-tracked template no longer ships any real-looking secret: the hard-coded `crm_app` DB password, the `crm:crm` owner creds, the `minioadmin/minioadmin` S3 keys, and `POSTGRES_PASSWORD=crm` are all replaced with `CHANGE_ME` placeholders. Real dev values live only in the gitignored `.env`. Also added the new `MFA_REQUIRED_FOR_PRIVILEGED=false` flag (§191, a feature flag not a secret) to both files. (The migration-hardcoded `crm_app` password above is a separate, still-open item.)
...
- [x] **DB-load of user on /refresh to enforce `is_active=false` instantly — DONE 2026-06-04.** `/refresh` now takes a `db` session and, after resolving the Redis refresh token to a user_id, re-loads the `User` row. If the account is missing or `is_active=False` it revokes ALL the user's sessions (kills the still-live refresh token so it can't keep minting) and returns 401 "Account is inactive" — previously a deactivated user kept getting fresh 15-min access tokens until the *next* protected call caught it via `get_current_user`. Also stamps the user's real `role` into the new access JWT instead of the old `role="unknown"` placeholder (the claim is cosmetic today since `get_current_user` re-reads role from the DB, but it was a latent trap). Covered by `test_refresh_rejects_deactivated_user` in `tests/test_refresh_rotation.py` (deactivate out-of-band → 401 + a retry is still 401 because sessions were burned).
...
- [x] **MFA mandatory for admin/manager — DONE 2026-06-05** (§191). Config flag `mfa_required_for_privileged` (env `MFA_REQUIRED_FOR_PRIVILEGED`, default off so a fresh install never locks out). "Privileged" is per-org: `deps.holds_privileged_role(db, user_id)` = holds admin/manager in ANY `OrgMembership`. **Enforcement choke point = `get_current_org_id`** — EVERY tenant-data endpoint depends on it (directly or via `get_current_org`/`get_current_membership`), while the MFA-enrollment + session endpoints use only `get_current_user`, so they stay reachable. A privileged un-enrolled user therefore gets a full session but every data route returns **403 `mfa_enrollment_required`** until `user.mfa_enabled` flips true (the membership lookup only runs while the flag is on AND the user hasn't enrolled, so the steady state pays nothing). `/login` gained a branch (after the existing MFA-challenge branch): privileged+no-MFA+flag → starts a full session + returns `MfaSetupRequired {mfa_setup_required, user, token}` + audits `user.login_mfa_setup_required`, so the SPA routes straight into forced enrollment instead of bouncing off a wall of 403s. Frontend: `isMfaSetupRequired` guard + 3-way `LoginResponse` union + `setMfaEnrollmentHandler` (in `request()`, a 403 with detail `mfa_enrollment_required` redirects to forced enrollment); login page redirects on the signal; new `/[locale]/mfa-setup` page (deliberately OUTSIDE the `(app)` layout so it makes no tenant-data calls and never trips its own gate) reuses `<MfaCard onEnabled>`; app layout registers the enrollment handler. 4 i18n keys (`mfa.forced*`) in all 7 locales. **6 tests** in `tests/test_mfa_enforcement.py` (policy-off no-op / login signals setup-required / blocked on data / gate clears once enrolled / non-privileged unaffected / MFA endpoints reachable while blocked); full backend suite **314/314**, tsc + eslint clean. NOT browser-tested (standing constraint) — live forced-redirect proven by reasoning + backend tests.
...
- [x] **UX nit — DONE 2026-06-04:** the partial-unique email index (`uq_leads_org_email_live` / `uq_customers_org_email_live`) now surfaces as a friendly **409** ("A {lead|customer} with this email already exists in this workspace") instead of a raw 500. Shared helper `app/api/_errors.py::raise_for_duplicate_email(exc, entity)` matches the `_org_email_live` constraint suffix and **re-raises any other `IntegrityError`** so genuine failures (FKs, other indices) are never masked. Wired into create (`flush`) **and** update on both leads and customers, each with a preceding `db.rollback()`. ruff + format clean; backend hot-reloaded healthy. **409 test added + real bug fixed 2026-06-04 (`tests/test_duplicate_email.py`, 8 tests: create-collision, case-insensitivity, update-collision, soft-deleted-reuse × lead/customer):** the UPDATE-path guard was originally placed around `db.commit()`, but `record_audit`/`record_activity`/`record_event`/`notify` each call `db.flush()`, so the pending UPDATE flushed (and the `IntegrityError` raised) *inside* `record_activity` — BEFORE the commit's `except` — escaping as a raw 500. Fixed by moving the guarded `db.flush()` to immediately after the `setattr` loop (before any audit/activity recording), mirroring the create path; the commit is now bare. Lesson: any unique-constraint guard must wrap the FIRST flush of the dirtied row, not the final commit, because the audit/activity/event/notify helpers flush eagerly.
...
- [x] Backend: smoke test every CRUD endpoint — `tests/test_crud_lead.py` (2 tests: full lifecycle create→list→get→patch→soft-delete→trash→restore→hard-delete; trust-boundary check that `organization_id` in body is ignored). (Per-resource Customer/Deal/Task lifecycle coverage added later — see the dedicated item below.)
- [x] Backend: audit log fires for each mutation — `tests/test_audit.py` (3 tests: lead.create + lead.update + user.login each leave an audit row with correct actor_id + organization_id). (Widened into a parametrized lifecycle matrix later — see the audit coverage matrix item below.)
- [x] **Per-resource lifecycle tests (Customer / Deal / Task) — DONE 2026-06-04.** `tests/test_crud_resources.py` drives each through the full create→list→get→patch→soft-delete→trash→restore→hard-delete pipeline (mirrors `test_crud_lead.py`), exercising the trash restore + hard_delete wiring per entity. Shape-aware helper handles both the customer `{items:[]}` cursor envelope and the plain-list deals/tasks; Task has no GET-by-id route so membership is asserted via list + trash; deal PATCH omits `If-Match` (lenient when absent). 3 tests, green. (Dashboard is a read-only aggregation, not a CRUD entity — no lifecycle to drive.)
- [x] **Audit coverage matrix — DONE 2026-06-04.** `tests/test_audit_matrix.py` parametrizes the four core entities (lead / customer / deal / task) through the full `create → update → soft_delete → restore → hard_delete` lifecycle, asserting the matching `{entity}.{action}` audit row fires at each step with the correct `actor_id` + `organization_id`. Reads rows via the BYPASSRLS `db` fixture so the assertion is independent of the request-path tenant GUC. 4 tests (one per entity), green. **Scope:** covers the CRUD/trash call sites in the four entity routers + `app/api/trash.py`; the remaining ~60 `record_audit` sites (Stripe webhooks, e-sign, quote/contract state machines, worker jobs, settings) stay behind their own fixtures and dedicated suites — folding them into one matrix would couple unrelated subsystems.
...
- [x] **e2e (Playwright) in CI — DONE 2026-06-04** (green run `26944287956`, `playwright-smoke` 3m54s). `.github/workflows/e2e.yml`: path-filtered (frontend/backend/compose/workflow), concurrency-cancel. Builds + starts only what the deterministic core smoke needs via `docker-compose.ci.yml` — db+redis+backend+frontend (NO ollama/minio/worker: the core path never scores or uploads and those clients are lazy). Waits on backend `/health` then frontend `/en/login`, then `npm ci` + `npx playwright install chromium` + `npx playwright test` (SCORE_E2E unset → AI-scoring test self-skips; core golden path register→login→create-lead→logout runs, ~27s). `docker-compose.ci.yml` also `!override`s the dev bind-mounts/anonymous volumes to `[]` so containers run from their freshly-built image fs (the base compose's anonymous `/app/.next` volume is created root-owned on a Linux runner → `next dev` EACCES; Docker Desktop masks the uid mismatch locally). **Two from-zero blockers fixed to get here:** (a) frontend EACCES on `/app/.next` (the volume override above, commit `c31a37f`); (b) **register 500 on fresh DB** — the audit_logs SELECT policy's system-context bypass tested `current_setting('app.current_org_id', true) = ''`, but on a never-set GUC `current_setting` returns SQL NULL, not '', so the register audit insert's RETURNING re-check failed RLS. Fixed by migration `f6a7b8c9d0e1` (`NULLIF(current_setting(...), '') IS NULL`, true for both NULL and ''; commit `d623254`). Same fresh-connection/from-zero divergence class as the empty-baseline bug (TD-1b) — a long-lived dev DB had masked it.
...
- [x] **Optimistic locking on Task + Customer — DONE 2026-06-04 (§321).** Migration `9a8b7c6d5e4f` adds `version int NOT NULL DEFAULT 0` to `customers` + `tasks` (hand-written, NOT autogenerated — mirrors `4d3d59906702`). `CustomerOut`/`TaskOut` expose `version`. `PATCH /api/customers/{id}` + `PATCH /api/tasks/{id}` accept `If-Match: <int>` (quoted form too), 412 on mismatch / 400 on malformed / v1-lenient missing-header, bump `version += 1` on success. The 30-line guard was extracted into a SHARED helper `app/api/_concurrency.py::check_if_match(entity=…, entity_id=…, current_version=…, if_match=…)` and deals.py refactored to call it (preserves its `deal.if_match_missing` log event + "version mismatch" 412 message — its tests still pass). `tests/test_resource_locking.py` parametrizes both entities × 6 cases (v0 start / lenient / correct-match / stale-412 / malformed-400 / concurrent one-wins). 35 green across locking + regression suites. Frontend still lenient (doesn't echo version yet) — that's the rollout posture, see strict-mode item below.
- [x] **Frontend echoes `version` (§322a) — DONE 2026-06-04.** `frontend/src/lib/api.ts`: `version: number` added to the `Customer`/`Deal`/`Task` interfaces; `updateCustomer`/`updateDeal`/`updateTask` (PATCH) + `moveDeal` (POST `/move`) take an optional `version?` param and send `If-Match: String(version)` only when defined (version 0 is sent — `!== undefined` guard, not truthiness). UI wired on all three entities: customer-edit page, tasks page, and the **pipeline board** (drag-drop passes the dragged deal's `version` to `moveDeal`; on 412 shows `common.versionConflict` and resyncs via `listDeals`). i18n `common.versionConflict` in 7 locales. There is no standalone deal-edit page — the board's `/move` is the only deal mutation path, so all deal writes now carry `If-Match`. vitest `tests/api.test.ts` "optimistic locking (If-Match)" block, 23/23 green. tsc + eslint clean.
- [x] **Strict mode — DONE 2026-06-04 (§322b).** `check_if_match` no longer has a lenient branch: a missing `If-Match` now raises **428 Precondition Required** (RFC 6585 §3) after logging `{entity}.if_match_missing` (kept as a signal). 412-on-mismatch / 400-on-malformed unchanged. Both locking suites flip the old "no-header proceeds" case to assert 428 (`test_patch_without_if_match_returns_428`, parametrized customer/task + the deal variant). Swept every other PATCH/`/move` caller in the suite that relied on leniency and added `If-Match: "0"` (or `created["version"]` where the row could be non-fresh): `test_crud_resources`, `test_activity_wiring`, `test_outbox`, plus the version-conditional helper in `test_audit_matrix` (leads carry no version) and `test_duplicate_email`. Full backend suite 307/307 (was 302+5fail); ruff check + format clean.
...
1. [x] **Company / Account (B2B entity)** — new `companies` table (org-scoped RLS, soft-delete); `Lead`/`Customer`/`Deal` gain a nullable `company_id`; Account detail page rolls up its contacts + deals + activity. **Do this first** — it's the schema decision the others build on (custom fields, tags, merge all want to target Company too), and retrofitting after data grows is painful. Keep `Customer` = the person, `Company` = the org.
2. [x] **Custom fields** — `custom_field_definitions` (org, entity_type, key, label, type, options, required, position) + a JSONB `custom_fields` column on lead/customer/deal/company. Wire into forms, detail, list columns, filters, and the CSV/XLSX importer. Highest adoption driver.
3. [x] **Tags / Segmentação** — `tags` (org, name, color) + tagging join (polymorphic or per-entity) + **saved segments** (stored filter JSON) + bulk tag/untag actions on list pages.
4. [x] **Duplicate merge** — detect candidates (email / phone / fuzzy name; the importer already dedupes on insert, this is the in-app *merge of two existing rows*) + a merge UI that re-parents deals/activities/notes/attachments to the survivor and soft-deletes the loser. Audit the merge.
5. [x] **Web-to-Lead (básico)** — public unauthenticated `POST /api/public/forms/{token}/submit` → creates a Lead in the token's org; an embeddable `<script>`/iframe snippet; **free anti-spam** (honeypot field + per-IP rate limit now; Cloudflare Turnstile / hCaptcha free tier later). The front door for organic inbound (and later the Ads connector).
...
| TD-31 | ~~**`crm_app` DB password hardcoded** in a migration.~~ **DONE 2026-06-08** — de-hard-coded (sourced from `APP_DATABASE_URL`/`CRM_APP_DB_PASSWORD`) + rotation migration `0862af48f9ca` (`ALTER ROLE`). Old value is in git history → rotate `APP_DATABASE_URL` per env (incl. Railway prod) and redeploy. | S | ✅ |
...
| ~~TD-44~~ | ~~**Frontend Docker image is a dev container.** `frontend/Dockerfile` does `npm ci` (incl. devDependencies) + `CMD npm run dev` — so the whole build toolchain ships in the scanned image, dragging in a recurring class of dev-tooling CVEs (node-tar/glob/minimatch/cross-spawn, suppressed in root `.trivyignore`).~~ — **DONE 2026-06-04** (PR #1, commits `0884525` + `b5a9152`). Multi-stage `frontend/Dockerfile` (`deps → dev → builder → runner`): the default `runner` stage ships only `next build` standalone output (no devDeps), so playwright/vitest/eslint + their node-tar/glob/cross-spawn transitives no longer land via the dev toolchain. The runner also `rm`s the `@esbuild` Go binary that next's standalone trace pulls in — it carried **2 CRITICAL + ~16 HIGH Go-stdlib CVEs** (not `.trivyignore`-covered) and the served `node server.js` never invokes it; that was the actual cause of the red Trivy gate. **`.trivyignore` was kept, NOT deleted** (the original plan): next's trace still copies tar/glob/minimatch/cross-spawn into the runner, so the file now documents only those standalone-traced transitives (header refreshed). docker-compose targets the new `dev` stage (source baked in) so local dev + CI e2e are unchanged. Verified: prod image builds (380 MB), serves `/pt/login` + `/en/login` 200; local Trivy exit 0 with `.trivyignore`; all 6 PR #1 checks green (`docker` + `e2e` included). | M | ✅ |
