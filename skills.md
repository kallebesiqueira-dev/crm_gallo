...
### P2 — engineering
- **TanStack Query** (kill the ~12 `useEffect`+`setState` fetch patterns).
- ~~**openapi-typescript** (kill FE/BE type drift).~~ ✅ DONE (#193, 2026-06-16) — `backend/openapi.json` (dumped from `app.openapi()` via `python -m scripts.dump_openapi`; `/openapi.json` is prod-disabled so the route can't be scraped) is the single source of truth; `frontend/src/lib/api-types.ts` generated by `openapi-typescript` (`npm run gen:api-types`), knip-ignored until imported (adopt incrementally). Two CI drift gates keep **app ↔ schema ↔ types** in lockstep: backend regenerates `openapi.json` & fails on app-mismatch; frontend (also triggered by `backend/openapi.json`) regenerates the types & fails on schema-mismatch. ⚠️ Always regenerate with the **pinned** deps — pydantic 2.13.x emits `additionalProperties: true` on open `dict` schemas, so a stale local image (e.g. 2.9.x) produces a diverging schema that fails CI.
...
- **Staging env** (host TBD). Post-deploy smoke ✅ `scripts/post_deploy_smoke.py` (health, security headers, auth-gating, error handling, SEO; run after each Railway redeploy).
