...
- [ ] **Strict mode**: flip `check_if_match` to require the header once the frontend echoes `version` on every PATCH (deals + customers + tasks); currently logs `{entity}.if_match_missing` as the rollout signal. Frontend does not yet send `If-Match` for any of the three.
